...
interface HomeFavoriteLocalLectureListProps {
  navigation: any;
  selectedSubZone: string[];
  categoryIdList: number[];
}

interface HomeFavoriteLocalLectureListStates {
  dropDowns: number[];
  data: any;
  favorites: number[];
...
export default class HomeFavoriteLocalLectureList extends React.Component<
  HomeFavoriteLocalLectureListProps,
  HomeFavoriteLocalLectureListStates
> {
  private beforeSelectedSubZone: string[];
  private beforeCategoryIdList: number[];
  ...
  constructor(props: any) {
    ...
    this.beforeSelectedSubZone = props.selectedSubZone;
    this.beforeCategoryIdList = props.categoryIdList;
    this.getLectureListApi(props.selectedSubZone, props.categoryIdList);
  }

  componentDidUpdate() {
    if (
      this.beforeSelectedSubZone === this.props.selectedSubZone &&
      this.beforeCategoryIdList === this.props.categoryIdList
    ) {
      return;
    }
    this.beforeSelectedSubZone = this.props.selectedSubZone;
  ...
  private async getLectureListApi(
    ...
    const params = {
      page: 0,
      size: 4,
      zoneId: selectedSubZone.length > 0 ? Zone.getId(selectedSubZone[0]) : '',
